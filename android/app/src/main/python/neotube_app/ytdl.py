...
class Download:
    ...
    def _download(self):
        ...
        try:
            ...
            def put_status(st):
                ...
                if st.get('filename'):
                    downloaded_filename = st.get('filename')
            ...
            def put_status_postprocessor(d):
                ...
                if d['postprocessor'] == 'MoveFiles' and d['status'] == 'finished':
                    ...
                    if not (
                        os.environ.get("NEOTUBE_ANDROID_FFMPEG_KIT", "").lower() in {"1", "true", "yes"}
                        and getattr(self.info, "download_type", "") == "audio"
                    ):
            ...
            android_ffmpeg = os.environ.get("NEOTUBE_ANDROID_FFMPEG")
            if android_ffmpeg and "ffmpeg_location" not in ytdl_params:
                ytdl_params["ffmpeg_location"] = android_ffmpeg
            if android_ffmpeg:
                log.info("Android FFmpeg location: %s", ytdl_params.get("ffmpeg_location"))
                try:
                    from yt_dlp.postprocessor.ffmpeg import FFmpegPostProcessor
                    FFmpegPostProcessor._version_cache[android_ffmpeg] = "3.3.2"
                    FFmpegPostProcessor._features_cache[android_ffmpeg] = {
                        "fdk": True,
                        "setts": False,
                        "needs_adtstoasc": False,
                    }
                except Exception as exc:
                    log.warning("Could not prime Android FFmpeg detection: %s", exc)
            ...
            if (
                ret == 0
                and os.environ.get("NEOTUBE_ANDROID_FFMPEG_KIT", "").lower() in {"1", "true", "yes"}
                and getattr(self.info, "download_type", "") == "audio"
                and downloaded_filename
            ):
                if not os.path.isabs(downloaded_filename):
                    downloaded_filename = os.path.join(self.download_dir, downloaded_filename)
                if not os.path.exists(downloaded_filename):
                    base_root, _reported_ext = os.path.splitext(downloaded_filename)
                    for source_ext in (".webm", ".m4a", ".mp4", ".opus", ".ogg", ".wav", ".flac"):
                        source_filename = f"{base_root}{source_ext}"
                        if os.path.exists(source_filename):
                            downloaded_filename = source_filename
                            break
                root, _ext = os.path.splitext(downloaded_filename)
                output_filename = f"{root}.{self.info.format}"
                if os.path.abspath(output_filename) != os.path.abspath(downloaded_filename):
                    log.info("Converting audio with FFmpegKit: %s -> %s", downloaded_filename, output_filename)
                    from java import jclass
                    bridge = jclass("com.neoapps.neotube.FFmpegKitBridge")
                    error = bridge.convertAudio(downloaded_filename, output_filename, self.info.format)
                    if error:
                        self.status_queue.put({"status": "error", "msg": str(error)})
                        return
                    try:
                        os.remove(downloaded_filename)
                    except OSError:
                        pass
                    self.status_queue.put({"status": "finished", "filename": output_filename})
    ...
    async def start(self, notifier):
        ...
        use_thread_worker = os.environ.get("NEOTUBE_ANDROID", "").lower() in {"1", "true", "yes"}
        if use_thread_worker:
            self.status_queue = queue.Queue()
            self.proc = threading.Thread(target=self._download, name="neotube-download", daemon=True)
        else:
    ...
    def close(self):
        ...
        if self.started() and hasattr(self.proc, "close"):
            self.proc.close()
    ...
    async def update_status(self):
        def relative_to_download_dir(path):
            try:
                return os.path.relpath(path, self.download_dir)
            except ValueError:
                # Windows raises when yt-dlp reports an intermediary file on a
                # different drive from DOWNLOAD_DIR. Keep status updates alive
                # and wait for MoveFiles to report the final destination.
                log.debug(
                    "Ignoring path outside download drive for %s: %s",
                    self.info.title,
                    path,
                )
                return None
        ...
        while True:
            ...
            if 'filename' in status:
                ...
                rel_name = relative_to_download_dir(fileName)
                if rel_name is None:
                    continue
            ...
            if 'chapter_file' in status:
                ...
                rel_path = relative_to_download_dir(chapter_file)
                if rel_path is None:
                    continue
            ...
            if 'subtitle_file' in status:
                ...
                rel_path = relative_to_download_dir(subtitle_output_file)
                if rel_path is None:
                    continue
