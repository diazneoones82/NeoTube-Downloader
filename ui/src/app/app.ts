...
export class App implements AfterViewInit, OnInit, OnDestroy {
  ...
  isAndroidApp = false;
  isIosApp = false;
  platformBrand = 'NEO Apps : Desktop';
  ...
  neotubeVersion: string | null = null;
  ...
  private readonly selectionCookiePrefix = 'neotube_selection_';
  ...
  constructor() {
    const nativeWindow = window as unknown as { NeoTubeAndroid?: unknown; NeoTubeIOS?: unknown };
    this.isAndroidApp = !!nativeWindow.NeoTubeAndroid;
    this.isIosApp = !!nativeWindow.NeoTubeIOS;
    this.platformBrand = this.isAndroidApp
      ? 'NEO Apps : Android'
      : this.isIosApp
        ? 'NEO Apps : iOS'
        : 'NEO Apps : Desktop';
    this.biometricEnabled = localStorage.getItem('neotube_biometric_enabled') === 'true';
    this.downloadType = this.cookieService.get('neotube_download_type') || 'video';
    this.codec = this.cookieService.get('neotube_codec') || 'auto';
    this.format = this.cookieService.get('neotube_format') || 'any';
    this.quality = this.cookieService.get('neotube_quality') || 'best';
    this.autoStart = this.cookieService.get('neotube_auto_start') !== 'false';
    this.splitByChapters = this.cookieService.get('neotube_split_chapters') === 'true';
    // Will be set from backend configuration, use empty string as placeholder
    this.chapterTemplate = this.cookieService.get('neotube_chapter_template') || '';
    this.clipStart = this.cookieService.get('neotube_clip_start') || '';
    this.clipEnd = this.cookieService.get('neotube_clip_end') || '';
    this.subtitleLanguage = this.cookieService.get('neotube_subtitle_language') || 'en';
    this.subtitleMode = this.cookieService.get('neotube_subtitle_mode') || 'prefer_manual';
    this.ytdlOptionsPresets = this.loadYtdlOptionsPresetsFromCookie();
    this.ytdlOptionsOverrides = this.cookieService.get('neotube_ytdl_options_overrides') || '';
    ...
    this.sortAscending = this.cookieService.get('neotube_sort_ascending') === 'true';

    const ci = parseInt(this.cookieService.get('neotube_check_interval') || '', 10);
  ...
  qualityChanged() {
    this.cookieService.set('neotube_quality', this.quality, { expires: this.settingsCookieExpiryDays });
    this.saveSelection(this.downloadType);
  ...
  downloadTypeChanged() {
    ...
    this.restoreSelection(this.downloadType);
    this.cookieService.set('neotube_download_type', this.downloadType, { expires: this.settingsCookieExpiryDays });
  ...
  codecChanged() {
    this.cookieService.set('neotube_codec', this.codec, { expires: this.settingsCookieExpiryDays });
    this.saveSelection(this.downloadType);
  }
  ...
  getConfiguration() {
    this.downloads.configurationChanged.pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      ...
      next: (config: any) => {
        ...
        if (!this.cookieService.check('neotube_check_interval')) {
  ...
  private loadYtdlOptionsPresetsFromCookie(): string[] {
    const jsonCookie = this.cookieService.get('neotube_ytdl_options_presets');
    if (jsonCookie) {
      try {
    ...
    const legacy = this.cookieService.get('neotube_ytdl_options_preset')?.trim();
  ...
  checkIntervalChanged() {
    this.cookieService.set('neotube_check_interval', String(this.checkIntervalMinutes), {
  ...
  getPreferredTheme(cookieService: CookieService) {
    let theme = 'auto';
    if (cookieService.check('neotube_theme')) {
      theme = cookieService.get('neotube_theme');
    }
  ...
  themeChanged(theme: Theme) {
    this.cookieService.set('neotube_theme', theme.id, { expires: this.settingsCookieExpiryDays });
    this.setTheme(theme);
  }
  ...
  formatChanged() {
    this.cookieService.set('neotube_format', this.format, { expires: this.settingsCookieExpiryDays });
    this.setQualities();
    this.saveSelection(this.downloadType);
    // Re-trigger custom directory change
    this.downloads.customDirsChanged.next(this.downloads.customDirs);
  ...
  autoStartChanged() {
    this.cookieService.set('neotube_auto_start', this.autoStart ? 'true' : 'false', { expires: this.settingsCookieExpiryDays });
  }

  splitByChaptersChanged() {
    this.cookieService.set('neotube_split_chapters', this.splitByChapters ? 'true' : 'false', { expires: this.settingsCookieExpiryDays });
  }
  ...
  chapterTemplateChanged() {
    ...
    this.cookieService.set('neotube_chapter_template', this.chapterTemplate, { expires: this.settingsCookieExpiryDays });
  }

  clipStartChanged() {
    this.cookieService.set('neotube_clip_start', this.clipStart, { expires: this.settingsCookieExpiryDays });
  }

  clipEndChanged() {
    this.cookieService.set('neotube_clip_end', this.clipEnd, { expires: this.settingsCookieExpiryDays });
  }

  subtitleLanguageChanged() {
    this.cookieService.set('neotube_subtitle_language', this.subtitleLanguage, { expires: this.settingsCookieExpiryDays });
    this.saveSelection(this.downloadType);
  }

  subtitleModeChanged() {
    this.cookieService.set('neotube_subtitle_mode', this.subtitleMode, { expires: this.settingsCookieExpiryDays });
    this.saveSelection(this.downloadType);
  }

  ytdlOptionsPresetsChanged() {
    this.cookieService.set(
      'neotube_ytdl_options_presets',
  ...
  ytdlOptionsOverridesChanged() {
    this.cookieService.set('neotube_ytdl_options_overrides', this.ytdlOptionsOverrides, { expires: this.settingsCookieExpiryDays });
  }
  ...
  openDownloadFolder(download: Download, filename = download.filename) {
    const androidBridge = (window as unknown as {
      NeoTubeAndroid?: { openFolder: (folder: string) => void }
    }).NeoTubeAndroid;
    if (androidBridge) {
      androidBridge.openFolder(download.folder || '');
      return;
    }
    this.downloads.openFolder({ ...download, filename }).subscribe((status: Status) => {
      if (status.status === 'error') {
        alert(status.msg || 'Unable to open download folder.');
      }
    });
  }
    ...
    this.http.get<{ 'yt-dlp': string, version: string }>(versionUrl)
      .subscribe({
        next: (data) => {
          ...
          this.neotubeVersion = data.version;
        },
        error: () => {
          this.ytDlpVersion = null;
          this.neotubeVersion = null;
        }
      });
  }
  ...
  biometricEnabledChanged() {
    localStorage.setItem('neotube_biometric_enabled', this.biometricEnabled ? 'true' : 'false');
  }
  ...
  toggleSortOrder() {
    ...
    this.cookieService.set('neotube_sort_ascending', this.sortAscending ? 'true' : 'false', { expires: this.settingsCookieExpiryDays });
    this.rebuildSortedDone();
  }
