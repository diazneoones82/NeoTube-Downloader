...
def _reject_unsafe_relative_path(value: str, field_name: str) -> None:
    if os.path.isabs(value) or pathlib.PureWindowsPath(value).drive:
        raise web.HTTPBadRequest(reason=f'{field_name} must be a relative path')
    if '..' in pathlib.PurePath(value).parts or '..' in pathlib.PureWindowsPath(value).parts:
        raise web.HTTPBadRequest(reason=f'{field_name} must not contain ".."')


def resolve_download_result_path(download_type: str, folder: str, filename: str) -> str:
    filename = str(filename or '').strip().replace('\\', '/')
    folder = str(folder or '').strip().replace('\\', '/')
    if not filename:
        raise web.HTTPBadRequest(reason='missing filename')

    _reject_unsafe_relative_path(filename, 'filename')
    if folder:
        _reject_unsafe_relative_path(folder, 'folder')

    download_dir = os.environ.get('DOWNLOAD_DIR') or config.DOWNLOAD_DIR
    audio_download_dir = os.environ.get('AUDIO_DOWNLOAD_DIR') or config.AUDIO_DOWNLOAD_DIR
    if isinstance(audio_download_dir, str) and audio_download_dir.startswith('%%'):
        audio_download_dir = download_dir
    base_dir = audio_download_dir if download_type == 'audio' else download_dir
    root = os.path.realpath(os.path.join(base_dir, folder))
    candidate = os.path.realpath(os.path.join(root, filename))
    try:
        if os.path.commonpath([root, candidate]) != root:
            raise web.HTTPBadRequest(reason='download path must stay inside the download folder')
    except ValueError as exc:
        raise web.HTTPBadRequest(reason='download path must stay inside the download folder') from exc
    return candidate
...
class NeotubeSubscriptionNotifier(SubscriptionNotifier):
...
submgr = SubscriptionManager(config, dqueue, NeotubeSubscriptionNotifier())
...
if config.YTDL_OPTIONS_FILE:
    if awatch is not None:
        ...
        log.warning("YTDL_OPTIONS_FILE watching is disabled because watchfiles is unavailable")
...
@routes.post(config.URL_PREFIX + 'open-folder')
async def open_folder(request):
    post = await _read_json_request(request)
    target = resolve_download_result_path(
        str(post.get('download_type') or ''),
        str(post.get('folder') or ''),
        str(post.get('filename') or ''),
    )
    folder = target if os.path.isdir(target) else os.path.dirname(target)
    if not os.path.isdir(folder):
        raise web.HTTPNotFound(reason='download folder was not found')
...
async def index(request):
    response = web.FileResponse(os.path.join(config.BASE_DIR, 'ui/dist/neotube/browser/index.html'))
    if 'neotube_theme' not in request.cookies:
        response.set_cookie('neotube_theme', config.DEFAULT_THEME)
    return response
...
async def version(request):
    return web.json_response({
        ...
        "version": os.getenv("NEOTUBE_VERSION", "dev")
    })
...
routes.static(config.URL_PREFIX, os.path.join(config.BASE_DIR, 'ui/dist/neotube/browser'))
try:
    app.add_routes(routes)
except ValueError as e:
    if 'ui/dist/neotube/browser' in str(e):
