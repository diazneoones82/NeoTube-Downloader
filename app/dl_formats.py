...
def get_format(download_type: str, codec: str, format: str, quality: str) -> str:
    ...
    if download_type == "video":
        ...
        if os.environ.get("NEOTUBE_ANDROID_NO_FFMPEG", "").lower() in {"1", "true", "yes"}:
            vres = f"[height<={quality}]" if quality not in ("best", "worst") else ""
            ext = "[ext=mp4]" if format in ("mp4", "ios") else ""
            if quality == "worst":
                return f"worst{ext}/best{ext}/best"
            return f"best{vres}{ext}/best{ext}/best"
...
def get_opts(
    ...
    no_ffmpeg = os.environ.get("NEOTUBE_ANDROID_NO_FFMPEG", "").lower() in {"1", "true", "yes"}
    android_ffmpeg_kit = os.environ.get("NEOTUBE_ANDROID_FFMPEG_KIT", "").lower() in {"1", "true", "yes"}

    postprocessors = []

    if download_type == "audio" and not no_ffmpeg and not android_ffmpeg_kit:
        ...
        if (
            format != "wav"
            and "writethumbnail" not in opts
            and os.environ.get("NEOTUBE_ANDROID", "").lower() not in {"1", "true", "yes"}
        ):
    ...
    if download_type == "video" and format in ("mp4", "ios") and not no_ffmpeg:
        postprocessors.append(
            {
