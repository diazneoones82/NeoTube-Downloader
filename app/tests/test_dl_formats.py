...
class DlFormatsTests(unittest.TestCase):
    ...
    def test_get_opts_all_audio_formats_use_ffmpeg_extract_audio(self):
        for fmt in ("m4a", "mp3", "opus", "wav", "flac"):
            with self.subTest(fmt=fmt):
                opts = get_opts("audio", "auto", fmt, "best", {})
                pp = next(p for p in opts["postprocessors"] if p["key"] == "FFmpegExtractAudio")
                self.assertEqual(pp["preferredcodec"], fmt)

    def test_get_opts_video_mp4_remuxes_to_mp4(self):
        opts = get_opts("video", "h264", "mp4", "best", {})
        self.assertIn(
            {"key": "FFmpegVideoRemuxer", "preferedformat": "mp4"},
            opts["postprocessors"],
        )

    def test_get_opts_video_ios_remuxes_to_mp4(self):
        opts = get_opts("video", "h265", "ios", "best", {})
        self.assertIn(
            {"key": "FFmpegVideoRemuxer", "preferedformat": "mp4"},
            opts["postprocessors"],
        )
