...
class ResolveDownloadResultPathTests(unittest.TestCase):
    def test_resolves_inside_download_folder(self):
        previous_download_dir = main.config.DOWNLOAD_DIR
        previous_audio_dir = main.config.AUDIO_DOWNLOAD_DIR
        previous_env_download_dir = os.environ.pop("DOWNLOAD_DIR", None)
        previous_env_audio_dir = os.environ.pop("AUDIO_DOWNLOAD_DIR", None)
        with tempfile.TemporaryDirectory() as tmp:
            main.config.DOWNLOAD_DIR = tmp
            main.config.AUDIO_DOWNLOAD_DIR = os.path.join(tmp, "audio")
            try:
                resolved = main.resolve_download_result_path("video", "clips", "movie.mp4")
            finally:
                main.config.DOWNLOAD_DIR = previous_download_dir
                main.config.AUDIO_DOWNLOAD_DIR = previous_audio_dir
                if previous_env_download_dir is not None:
                    os.environ["DOWNLOAD_DIR"] = previous_env_download_dir
                if previous_env_audio_dir is not None:
                    os.environ["AUDIO_DOWNLOAD_DIR"] = previous_env_audio_dir
        self.assertEqual(resolved, os.path.realpath(os.path.join(tmp, "clips", "movie.mp4")))

    def test_prefers_runtime_download_folder(self):
        previous_download_dir = main.config.DOWNLOAD_DIR
        previous_env_download_dir = os.environ.get("DOWNLOAD_DIR")
        with tempfile.TemporaryDirectory() as tmp:
            main.config.DOWNLOAD_DIR = os.path.join(tmp, "config")
            os.environ["DOWNLOAD_DIR"] = os.path.join(tmp, "runtime")
            try:
                resolved = main.resolve_download_result_path("video", "", "movie.mp4")
            finally:
                main.config.DOWNLOAD_DIR = previous_download_dir
                if previous_env_download_dir is None:
                    os.environ.pop("DOWNLOAD_DIR", None)
                else:
                    os.environ["DOWNLOAD_DIR"] = previous_env_download_dir
        self.assertEqual(resolved, os.path.realpath(os.path.join(tmp, "runtime", "movie.mp4")))

    def test_rejects_parent_traversal(self):
        with self.assertRaises(main.web.HTTPBadRequest):
            main.resolve_download_result_path("video", "", "../secret.txt")
